...
namespace Adaptive.ReactiveTrader.Server.Pricing
{
    public sealed class PriceSource : IDisposable
    {
        ...
        private static readonly IScheduler Scheduler = ConcurrencyService.CreateEventScheduler("Pricing");
        ...
        private static IObservable<Unit> CreatePriceTrigger(bool delayPeriods)
        {
            ...
            return Observable.Interval(TimeSpan.FromSeconds(0.5), Scheduler).Delay(TimeSpan.FromMilliseconds(Random.Next(101)), Scheduler).Select(_ => Unit.Default);
        }
